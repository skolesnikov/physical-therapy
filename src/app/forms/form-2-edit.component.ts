...
@Component({
    templateUrl: './form-2-edit.component.html',
    styleUrls: ['./form.component.css']
})
export class FormTwoEditComponent implements OnInit, OnDestroy {
    form: IForm;
    patient: IPatient;
    patients: IPatient[];
    patientNames: string[] = [];
    ...
    getForm(id: number, patientId: string) {
        if (id === 0) {
            this.formService.getNewForm(2, patientId).subscribe(
                        form => {this.form = form; 
    ...
    saveForm(): void {
        ...
        this.formService.saveForm(this.form)
            .subscribe(
                form => {this.router.navigate([`/form-2-edit/${form.id}`]);},
    ...
    onPrint(): void {
        this.formService.saveForm(this.form)
            .subscribe(
                form => {this.form = form; 
                        this.router.navigate([`/form-2-print/${this.form.id}`]);},
                (error: any) => this.errorMessage = <any>error);
    }
}