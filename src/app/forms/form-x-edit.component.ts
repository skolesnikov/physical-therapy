...
@Component({
    templateUrl: './form-x-edit.component.html',
    styleUrls: ['./form.component.css']
})
export class FormXEditComponent implements OnInit, OnDestroy {
    form: IForm;
    patient: IPatient;
    patients: IPatient[];
    patientNames: string[] = [];
    ...
    ngOnInit(): void {
        ...
        this.sub = this.route.params.subscribe(
            params => {
                let formTypeId = +params['formTypeId'];
                let id = +params['id'];
                let patientId = params['patientId'];
                this.getForm(formTypeId, id, patientId);
        }); 
    ...
    getForm(formTypeId: number, id: number, patientId: string) {
        if (id === 0) {
            this.formService.getNewForm(formTypeId, patientId).subscribe(
                        form => {this.form = form; 
    ...
    saveForm(): void {
        ...
        this.formService.saveForm(this.form)
            .subscribe(
                form => {this.router.navigate([`/form-x-edit/${form.formTypeId}/${form.id}`]);},
    ...
    onPrint(): void {
        this.formService.saveForm(this.form)
            .subscribe(
                form => {this.form = form; 
                        this.router.navigate([`/form-x-print/${this.form.id}`]);},
                (error: any) => this.errorMessage = <any>error);
    }
}